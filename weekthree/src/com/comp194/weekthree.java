...
public class weekthree extends Activity {
	
	private static String DEBUGTAG = "weekthree"; // our debug tag so we can use logcat filters
    ...
    private Handler handler = new Handler(){
    	public void handleMessage(Message msg) {
    		...
    		case 0: 
    			Log.i(weekthree.DEBUGTAG, "Fetching Photos");
    		...
    		case 1:
    			Log.i(weekthree.DEBUGTAG, "Updating Gallery");
		...
		public void onLocationChanged(final Location arg0) {
			...
			Thread t = new Thread() {
				public void run(){
					...
					Log.i(weekthree.DEBUGTAG, "Fetch geocode");
					try {
						...
						Log.e(weekthree.DEBUGTAG, e.getMessage());
						handler.sendEmptyMessage(0);
						return;
					}
					
					Log.i(weekthree.DEBUGTAG, "Extract woeid");
					try {
						obj = new JSONObject(content);
						results = ((JSONObject) obj.get("places")).getJSONArray("place");
						
						if(results.length() == 0){
							Log.i(weekthree.DEBUGTAG, "No places matched!");
							handler.sendEmptyMessage(0);
							return;
						}
					...
					Log.i(weekthree.DEBUGTAG, "Fetch urls");
					content = "";
					data = "";
					try {
					...
					Log.i(weekthree.DEBUGTAG, "JSON urls");
					try {
						obj = new JSONObject(content);
					...
					Log.i(weekthree.DEBUGTAG, "flickr run complete");
					handler.sendEmptyMessage(0);
				}
			};
        	...
        	Thread t = new Thread() {
        		public void run(){
        			...
        			for(int i=0; i < urlList.size(); i++){
        				...
        				Log.i(weekthree.DEBUGTAG, "fetching " + i + " / " + urlList.size());
        				...
        				try {
        					...
        					Log.e(weekthree.DEBUGTAG, e.getMessage(), e);
        				}
